...
if (!empty($_GET["start"]) && !empty($_GET["end"])){
	$date_filter_condition = " and bt.created_date between '".date("Y-n-d", strtotime($_GET["start"]))." 00:00:00 ' and '".date("Y-n-d", strtotime($_GET["end"]))." 23:59:59' ";
}
...
<?php } ?>
								...
								<div class="panel panel-primary">
									<div class="panel-heading">
										...
										<div class="col-xs-9 text_right">
											...
											<div class="huge" id="total_cart"></div>
											<div>Add to cart</div>
										</div>
										<hr class="hr">
									</div>
								...
								<div class="panel panel-cust">
									<div class="panel-heading">
										...
										<div class="col-xs-9 text_right">
											...
											<div class="huge" id="total_purchased"></div>
											<div>Purchased</div>
										</div>
										<hr class="hr">
									</div>
								...
								<div class="panel panel-green">
									<div class="panel-heading">
										...
										<div class="col-xs-9 text_right">
											...
											<div class="huge" id="totalt_purchased"></div>
						...
						<div class="result_box_main">
							<div class="option home-border text-capitalize result_data" style="display:none;">
								<?php
									$sql_added_in_cart = "SELECT bt.bid, bt.pid, COUNT( bt.add_to_cart ) AS tot_added, bm.b_title ".
												"FROM banner_transacrion AS bt LEFT JOIN banner_mst AS bm ON bt.bid = bm.bid ".
												"where bt.add_to_cart=1 and bt.shop='".$shop."' $date_filter_condition ".
												"Group by bt.pid ORDER BY tot_added DESC, bt.bid ASC";
								...
								<div class="list_addtocart result_list" style="display:none;">
									...
									<table>
										...
										<?php 
											$total_add_cart_cost= 0;
											while($row_addtocart = mysql_fetch_assoc($res_added_in_cart)){ 
												$p_name = " - ";
												$p_id = explode('||||',$row_addtocart["pid"]);
												$bid = base64_encode($row_addtocart["bid"]);
												$product_data = array();
												try{
													$product_data = $sc->call('GET', "/admin/products/".$p_id[0].".json");
												} catch (exception $e) {
												...
												if(count($product_data) > 0){
													...
													foreach($product_data["variants"] as $variant) {
														if ($variant["id"] == $p_id[1]){
															$p_price = $variant["price"];
															$total_add_cart_cost += $p_price;
															break;
														}
													}
								...
								<?php 
									...
									$sql_purchased = "SELECT bt.bid, bt.pid, COUNT( bt.is_purchased ) AS tot_purchased, bm.b_title ".
												"FROM banner_transacrion AS bt LEFT JOIN banner_mst AS bm ON bt.bid = bm.bid ".
												"where bt.is_purchased=1 and bt.shop='".$shop."' $date_filter_condition ".
												"Group by bt.pid ORDER BY tot_purchased DESC, bt.bid ASC";
									$res_purchased = mysql_query($sql_purchased);
								...
								<div class="list_purchased result_list" style="display:none;">
									...
									<table>
										...
										<?php 
											$total_p_add_cart_cost =0;
											while($row_purchased = mysql_fetch_assoc($res_purchased)){ 
												$p_name = " - ";
												$p_id = explode('||||',$row_purchased["pid"]);
												...
												if(count($product_data) > 0){
													...
													foreach($product_data["variants"] as $variant) {
														if ($variant["id"] == $p_id[1]){
															$pp_price = $variant["price"];
															$total_p_add_cart_cost += $pp_price;
														}
													}
								...
								<?php 
									...
									$sql_max_pro_tran = "SELECT bt.bid, bt.pid, COUNT( bt.is_purchased ) AS tot_purchased, bm.b_title ".
												"FROM banner_transacrion AS bt LEFT JOIN banner_mst AS bm ON bt.bid = bm.bid ".
												"where bt.is_purchased=1 and bt.shop='".$shop."' $date_filter_condition ".
												"Group by bt.pid ORDER BY tot_purchased DESC, bt.bid ASC";
								...
								<div class="list_maxprotransaction result_list" style="display:none;">
									...
									<table>
										...
										<?php 
											$total_pt_add_cart_cost = 0;
											while($row_max_pro_tran = mysql_fetch_assoc($res_max_pro_tran)){ 
												$p_name = " - ";
												$p_id = explode('||||',$row_max_pro_tran["pid"]);
												...
												if(count($product_data) > 0){
													...
													foreach($product_data["variants"] as $variant) {
														if ($variant["id"] == $p_id[1]){
															$pt_price = $variant["price"];
															$total_pt_add_cart_cost += $pt_price;
														}
													}
...
<script>
		...
		$("#total_cart").html("$<?php echo $total_add_cart_cost; ?>");
		$("#total_purchased").html("$<?php echo $total_p_add_cart_cost; ?>");
		$("#totalt_purchased").html("$<?php echo $total_pt_add_cart_cost; ?>");
